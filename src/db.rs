...
impl Database {
    ...
    pub fn save_workspace(&self, ws: &Workspace) -> Result<()> {
        let name = self
            .conn
            .query_row(
                "SELECT EXISTS(SELECT name FROM workspaces WHERE name = ?1 AND id != ?2)",
                [&ws.name, &ws.id],
            ...
            .map(|exists| match exists {
                true => {
                    ...
                    self.conn
                        .query_one("SELECT name FROM workspaces WHERE id = ?1", [&ws.id], |row| {
        ...
        self.conn
            .execute(
                "INSERT INTO workspaces (id, name, shell, default_profile) VALUES (?1, ?2, ?3, ?4)",
    ...
    pub fn list_workspaces(&self) -> Result<Option<Vec<Workspace>>> {
        let mut stmt = self.conn.prepare("SELECT * FROM workspaces ")?;
        let mut rows = stmt.query([])?;
        let mut wslist = Vec::new();

        while let Some(row) = rows.next()? {
            let id: String = row.get(0)?;

            let new_ws = Ok::<Workspace, anyhow::Error>(Workspace {
                id: id.clone(),
                name: row.get(1)?,
                shell: row.get(2)?,
                default_profile: row.get(3)?,
                profiles: self.get_profiles(id.as_str())?,
            })?;
    ...
    pub fn get_workspace(&self, id: &str) -> Result<Option<Workspace>> {
        let mut stmt = self
            .conn
            .prepare("SELECT name, shell, default_profile FROM workspaces WHERE id = ?1")?;
        ...
        if let Some(row) = rows.next()? {
            ...
            let profiles = self.get_profiles(id)?;
    ...
    fn get_profiles(&self, id: &str) -> Result<Vec<Profile>> {
        let mut stmt = self
            .conn
    ...
    pub fn get_id_by_name(&self, name: &str) -> Result<Option<String>> {
        let mut stmt = self.conn.prepare("SELECT id FROM workspaces WHERE name = ?1")?;
